...
const FEEDBACK = {
  ...
  whatWentWell: ['Clear problem statement', 'Logical flow', 'Strong explanation'],
  improveNext: ['Strengthen your conclusion', 'Reduce repetition', 'Slow down during introduction'],
  fillerWords: [
    { word: 'um', count: 4 },
    { word: 'like', count: 2 },
...
export default function AttemptFlowScreen() {
  ...
  const [phase, setPhase] = useState(view === 'feedback' ? 'feedback' : 'recording');
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => setProcessingStep((prev) => prev + 1), 700);
    return () => clearTimeout(timeout);
      ...
      <ScreenContainer contentStyle={styles.centeredContent}>
        ...
        <View style={styles.recordingBody}>
          <View style={styles.statusRow}>
            ...
            <Text style={styles.statusLabel}>{isPaused ? 'PAUSED' : 'RECORDING'}</Text>
          </View>
        ...
        <View style={styles.offlineNote}>
          ...
          <Text style={styles.offlineNoteText}>Recording continues to work without internet.</Text>
        </View>
      </ScreenContainer>
    );
  }
    ...
    return (
      <ScreenContainer>
        ...
        <View style={styles.reviewCard}>
          ...
          <Text style={styles.reviewPosition}>0:00 / {formatDuration(elapsedSeconds)}</Text>
        </View>

        <PrimaryButton label="RECORD AGAIN" icon="mic" pill onPress={handleRecordAgain} />
        <TouchableOpacity onPress={() => setShowDeleteRecording(true)} style={styles.textButton}>
        ...
        {PROCESSING_STEPS.map((step, index) => {
          ...
          return (
            <View key={step} style={styles.processingRow}>
              ...
              />
              <Text style={[styles.processingLabel, !done && !active && styles.processingLabelPending]}>
                {step}
              </Text>
            </View>
          );
  ...
  if (phase === 'transcript') {
    return (
      <ScreenContainer>
        ...
        <TouchableOpacity onPress={() => setSavedChanges(true)} style={styles.textButton}>
          <Text style={styles.textButtonLabel}>{savedChanges ? 'Changes saved' : 'Save Changes'}</Text>
        </TouchableOpacity>
  ...
  return (
    <ScreenContainer>
      ...
      <Text style={styles.scoreDelta}>
        ↑ +{(FEEDBACK.score - FEEDBACK.previousScore).toFixed(1)} from Attempt {FEEDBACK.previousAttempt}
      </Text>
      ...
      <View style={styles.nextStepCard}>
        ...
        <PrimaryButton label="RECORD ANOTHER ATTEMPT" icon="mic" pill onPress={handleRecordAnother} />
      </View>
      ...
      <View style={styles.listCard}>
        ...
        >
          <Text style={styles.saveNoteButtonText}>{noteSaved ? 'Note saved' : 'Save Note'}</Text>
        </TouchableOpacity>
      </View>

      <TouchableOpacity onPress={handleBackToSession} style={styles.backToSessionLink}>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <Text style={styles.modalDescription}>
              Your SpeakSharp rating is a coaching score designed to help you track improvement
              between attempts. It is not a prediction of your university mark.
            </Text>
            <PrimaryButton label="Done" onPress={() => setShowScoreInfo(false)} />
          </View>
        </View>
      </Modal>
    </ScreenContainer>
